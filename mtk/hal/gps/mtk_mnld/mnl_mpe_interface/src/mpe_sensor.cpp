...
unsigned char mpe_sensor_start(SENSOR_USER_ID id, SENSOR_TYPE mpe_type, UINT32 periodInMs)
{
    ...
    if (sensorContext[id].queue->enableSensor(
        sensorContext[id].sensorHandle[mpe_type], periodInMs*1000, 0) != Result::OK) {
...
unsigned char mpe_sensor_stop(SENSOR_USER_ID id, SENSOR_TYPE mpe_type)
{
    ...
    if (sensorContext[id].queue->disableSensor(
        sensorContext[id].sensorHandle[mpe_type]) != Result::OK) {
...
unsigned char mpe_sensor_check_mnl_response(void)
{
    ...
    if((delta_input_time > 2.f && mpe_sensor_get_listen_mode() == MPE_START_MODE && !phone_time_reset) || phone_time_reset)
    {
        mpe_sensor_set_listen_mode(MPE_IDLE_MODE);
