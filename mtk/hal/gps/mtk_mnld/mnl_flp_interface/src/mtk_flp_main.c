...
INT32 mtk_gps_ofl_sys_mnl_offload_callback(MTK_GPS_OFL_CB_TYPE type, UINT16 length, UINT8 *data) {
    ...
    switch (type) {
        case MTK_GPS_OFL_CB_HEART_BEAT:
        {
            if (length >= 12) {
                offset = 0;
                first_heartbeat = get_int((char*)data, &offset);
                heartbeat_idx   = get_int((char*)data, &offset);
                ms_to_next      = get_int((char*)data, &offset);
...
int mtk_hal2flp_main_hdlr(int fd, flp2mnl_interface* hdlr) {
    ...
    len = get_binary(buff, &offset, data_recv);
