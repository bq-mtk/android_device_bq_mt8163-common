...
int mtk_flp2mnl_process(MTK_FLP_MSG_T *prmsg) {
    ...
    //LOGD("mtk_flp2mnl_process, recv prmsg, type:0x%02x, len:%u status:%u\r\n", prmsg->type, prmsg->length, fgMnldStatus);
    ...
    switch(prmsg->type) {
        case MTK_FLP_MSG_HAL_INIT_CMD:
            ...
            if(flp2mnl_msg != NULL ) {
                flp2mnl_msg->type = MNLD_FLP_TYPE_FLP_ATTACH;
                flp2mnl_msg->length = 0;
                flp2mnl_msg->session = 0;
        ...
        case MTK_FLP_MSG_HAL_DIAG_INJECT_DATA_NTF:
            ...
            if (fgMnldStatus == MNL_STATUS_OFL_LINK_OPEN_DONE) {
                ...
                if(flp2mnl_msg != NULL) {
                    ...
                    LOGD("Send to OFL FLP,message type:0x%02x, len:%u", prmsg->type, (flp2mnl_msg->length + sizeof(MTK_FLP_MNL_MSG_HEADER_T)));
                    flp2mnl_hdlr(flp2mnl_msg);
                    free(flp2mnl_msg);
                }
            } else {
                LOGD("FLP_DBG: mnld state expect 8, get %u", fgMnldStatus);
            }
            break;
...
int mnl2flp_hdlr(MTK_FLP_MNL_MSG_T* prmsg) {
    ...
    switch(prmsg->type) {
        case MNLD_FLP_TYPE_MNL_BOOTUP:
            if (fgMnldStatus != MNL_STATUS_NONE) {
                ...
                if(flp2mnl_msg != NULL) {
                    ...
                    LOGE("flp attach malloc failed");
                }
            } else {
                LOGD("normal mnl bootup");
            }
            break;
        ...
        case MNLD_FLP_TYPE_HBD_GPS_LOCATION:
            ...
            if(mtk_flp_gpsloc2flploc(gps_loc, &flp_loc) != MTK_FLP_SUCCESS) {
        ...
        case MNLD_FLP_TYPE_OFL_LINK_RECV:
            ...
            switch(cnn2flp_msg->type) {
                ...
                case MTK_FLP_MSG_OFL_REPORT_LOC_NTF:
                    //check location size
                    if(cnn2flp_msg->length >= sizeof(MTK_FLP_LOCATION_T)) {
