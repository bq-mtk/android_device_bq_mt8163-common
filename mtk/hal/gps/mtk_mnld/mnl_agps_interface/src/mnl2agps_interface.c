...
void agps2mnl_hdlr(int fd, agps2mnl_interface* mnl_interface) {
    ...
    case MNL_AGPS_TYPE_SETTINGS_SYNC: {
        ...
        agps_settings.a_gps_satellite_enable = get_int(buff, &offset);
        agps_settings.a_beidou_satellite_enable = get_int(buff, &offset);
        agps_settings.a_galileo_satellite_enable = get_int(buff, &offset);
        agps_settings.lppe_enable = get_int(buff, &offset);
    ...
    case MNL_AGPS_TYPE_VZW_DEBUG_SCREEN_OUTPUT: {
        const char* str = get_string(buff, &offset);
    ...
    case MNL_AGPS_TYPE_LPPE_ASSIST_COMMON_IONO: {
        ...
        int lppe_len=get_int(buff, &offset);
