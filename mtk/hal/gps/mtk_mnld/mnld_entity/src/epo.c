...
static int epo_DAT_file_merge(int file_length) {
    ...
    strncat(gps_epo_data_file_name, "/data/misc/gps/", EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_data_file_name));
...
static void check_epo_file_exist(void) {
    ...
    for(i = 0; i< gps_epo_download_piece; ++i) {
        ...
        strncat(gps_epo_data_file_name, "/data/misc/gps/", EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_data_file_name));
        strncat(gps_epo_data_file_name, gps_epo_file_name, EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_data_file_name));
        strncat(gps_epo_md5_file_name, "/data/misc/gps/", EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_md5_file_name));
...
void getEpoUrl(char* filename, char* url) {
    ...
    if (counter <= 1) {
        strncat(url, "https://mgepodownload.mediatek.com/", GPS_EPO_URL_LEN - strlen(url));
    } else {
        strncat(url, "https://mepodownload.mediatek.com/", GPS_EPO_URL_LEN -strlen(url));
    }
...
static int mtk_gps_epo_server_data_is_changed() {
    ...
    strncat(gps_epo_data_file_name_start, "/data/misc/gps/", EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_data_file_name_start));
    ...
    if (gps_epo_file_count > 0) {
        ...
        strncat(gps_epo_data_file_name_end, "/data/misc/gps/", EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_data_file_name_end));
    ...
    if (uTime_start >= ((24*60*60) + uTime_end)) {
        ...
        for (i = gps_epo_file_count; 0 <= i; i--) {
            ...
            gps_download_epo_file_name(i);
            strncat(gps_epo_piece_file_name, "/data/misc/gps/", EPO_FILE_NAME_MAX_SIZE - strlen(gps_epo_piece_file_name));
