...
static int get_mtklog_path(char *logpath) {
    ...
    if (ptr) {
        ...
        MNLD_STRNCPY(logpath, "/data/misc/gps/", GPS_DEBUG_LOG_FILE_NAME_MAX_LEN);
        return 0;
    }
    ...
    if (len <= GPS_DEBUG_LOG_FILE_NAME_MAX_LEN-7) {
        ...
        if (0 != access(mtklogpath, F_OK)) {    // if mtklog dir is not exit, mkdir
             ...
             if (mkdir(mtklogpath, 0775) == -1) {
                 MNLD_STRNCPY(logpath, "/data/misc/gps/", GPS_DEBUG_LOG_FILE_NAME_MAX_LEN);  // if mkdir fail, set default path
